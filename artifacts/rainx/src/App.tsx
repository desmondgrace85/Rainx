...
export default function App() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      if (mounted) setAccount(data.session?.user ? { id: data.session.user.id, email: data.session.user.email } : null);
    });
    const { data: listener } = supabase.auth.onAuthStateChange((_event, session) => {
      if (mounted) setAccount(session?.user ? { id: session.user.id, email: session.user.email } : null);
    });
    return () => {
      mounted = false;
