...
function Metric({label,value,delta,highlight=false}){const up=Number(delta||0)>=0;return <div style={{border:`1px solid ${highlight?"#65A9CF":LINE}`,borderRadius:16,padding:"18px 16px 14px",background:highlight?"#F7FCFF":CARD,minHeight:112,boxSizing:"border-box"}}><div style={{fontSize:15,fontWeight:650}}>{label}</div><div style={{fontSize:31,lineHeight:1.08,fontWeight:800,letterSpacing:-1.2,marginTop:8}}>{typeof value==="string"?value:fmt(value,true)}</div><div style={{marginTop:8,fontSize:13,color:delta===undefined?MUTED:up?BLUE:"#C04444",fontWeight:700}}>{delta===undefined?"All time":`${up?"↑":"↓"} ${Math.abs(delta).toFixed(1)}%`}</div></div>}
function Section({title,children}){return <section style={{background:CARD,borderRadius:18,padding:"20px 18px",marginBottom:14}}><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:16}}><h2 style={{margin:0,fontSize:21,lineHeight:1.15,fontWeight:800,letterSpacing:-.5}}>{title}</h2><Info size={17} color={MUTED}/></div>{children}</section>}
function Pills({items,active,onChange}){return <div style={{display:"flex",gap:10,overflowX:"auto",scrollbarWidth:"none",paddingBottom:2}}>{items.map(x=><button key={x} onClick={()=>onChange(x)} style={{flexShrink:0,border:0,borderRadius:24,padding:"10px 18px",background:active===x?"#000":"#E9EAEC",color:active===x?"#fff":"#73777C",fontFamily:FONT,fontWeight:700,fontSize:15}}>{x}</button>)}</div>}
...
export default function AnalyticsScreen({account,onBack}){
  ...
  return <div style={{position:"fixed",inset:0,zIndex:100,background:BG,color:INK,fontFamily:FONT,overflow:"hidden"}}><div style={{height:"100%",overflowY:"auto",overflowX:"hidden",overscrollBehavior:"none",WebkitOverflowScrolling:"touch",touchAction:"pan-y",scrollbarWidth:"none",paddingBottom:30}}>
    <header style={{height:68,display:"flex",alignItems:"center",justifyContent:"center",position:"sticky",top:0,zIndex:20,background:"rgba(245,245,245,.97)",backdropFilter:"blur(8px)"}}><button onClick={onBack} style={{position:"absolute",left:18,border:0,background:"transparent",padding:6,color:INK}}><ArrowLeft size={29}/></button><h1 style={{margin:0,fontSize:25,fontWeight:800,letterSpacing:-.8}}>Analytics</h1></header>
    <div style={{display:"flex",overflowX:"auto",borderBottom:`1px solid ${LINE}`,scrollbarWidth:"none"}}>{TABS.map(x=><button key={x} onClick={()=>setTab(x)} style={{flex:"0 0 auto",minWidth:tab==="Viewers"?92:110,height:52,border:0,borderBottom:tab===x?"3px solid #111":"3px solid transparent",background:"transparent",color:tab===x?INK:"#858A91",fontFamily:FONT,fontWeight:tab===x?800:650,fontSize:17}}>{x}</button>)}</div>
    <div style={{padding:"24px 15px 0"}}><Pills items={["7 days","28 days","60 days","365 days"]} active={period} onChange={setPeriod}/>
      {tab==="Overview"&&<><Section title="Key metrics"><div style={{color:MUTED,fontSize:15,marginTop:-9,marginBottom:16}}>{new Date(Date.now()-(daysFor(period)-1)*864e5).toLocaleDateString("en",{month:"short",day:"numeric"})} - {new Date().toLocaleDateString("en",{month:"short",day:"numeric"})}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}><Metric label="Post views" value={d.views} delta={d.deltaViews} highlight/><Metric label="Profile views" value={d.profileViews}/><Metric label="Likes" value={d.likes}/><Metric label="Comments" value={d.comments}/><Metric label="Shares" value={d.shares}/><Metric label="Est. rewards" value="$0.00"/></div><div style={{marginTop:20}}><SmoothChart points={d.daily}/></div></Section>
        <Section title="Traffic sources">{(d.source.length?d.source:SOURCES.map(([key,label])=>({key,label,pct:0}))).map(r=><div key={r.key} style={{marginBottom:17}}><div style={{display:"flex",justifyContent:"space-between",fontSize:15,marginBottom:8}}><span>{r.label}</span><strong>{r.pct.toFixed(1)}%</strong></div><div style={{height:19,borderRadius:4,background:BAR,overflow:"hidden"}}><div style={{height:"100%",width:`${Math.min(100,r.pct)}%`,background:r.key==="for_you"?GOLD:BLUE,borderRadius:4}}/></div></div>)}</Section>
        <Section title="Search queries">{d.source.find(x=>x.key==="search")?.pct? <div style={{color:MUTED,fontSize:14}}>Search traffic is being tracked. Detailed query text will appear when query-level search events are recorded.</div>:<div style={{color:MUTED,fontSize:14}}>Search query tracking will appear here as people discover your posts through search.</div>}</Section></>}
      {tab==="Content"&&<Section title="Top content">{!d.top.length?<div style={{color:MUTED,fontSize:14}}>Post something to see your content performance.</div>:d.top.map((p,i)=><div key={p.id} style={{display:"flex",gap:12,alignItems:"center",padding:"13px 0",borderBottom:i<d.top.length-1?`1px solid ${LINE}`:"none"}}><div style={{width:34,height:34,borderRadius:8,background:"#F0F1F2",display:"grid",placeItems:"center",fontWeight:800}}>{i+1}</div><div style={{minWidth:0,flex:1}}><div style={{fontSize:14,fontWeight:650,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{p.text||"Media post"}</div><div style={{fontSize:12,color:MUTED,marginTop:3}}>{fmt(p.views)} views · {fmt(p.likes_count)} likes · {fmt(p.comments_count)} comments</div></div></div>)}</Section>}
      {(tab==="Viewers"||tab==="Followers")&&<><Section title="Key metrics"><div style={{color:MUTED,fontSize:15,marginTop:-9,marginBottom:16}}>{new Date(Date.now()-(daysFor(period)-1)*864e5).toLocaleDateString("en",{month:"short",day:"numeric"})} - {new Date().toLocaleDateString("en",{month:"short",day:"numeric"})}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>{tab==="Viewers"?<><Metric label="Total viewers" value={insightData?.total||0} highlight/><Metric label="New viewers" value={insightData?.new||0}/></>:<><Metric label="Total followers" value={insightData?.total||0} highlight/><Metric label="Net followers" value={insightData?.net||0}/></>}</div><div style={{marginTop:20}}><SmoothChart points={insightData?.daily||[]}/></div></Section>
        <Section title={tab==="Viewers"?"Viewer insights":"Follower insights"}><Pills items={["Gender","Age","Locations"]} active={insight} onChange={setInsight}/>{insight==="Locations"?<div style={{marginTop:18}}>{insightData?.locations?.length?insightData.locations.map(([name,count])=><div key={name} style={{marginBottom:15}}><div style={{display:"flex",justifyContent:"space-between",fontSize:15,marginBottom:7}}><span>{name}</span><strong>{((count/(insightData.locations[0]?.[1]||1))*100).toFixed(1)}%</strong></div><div style={{height:16,background:BAR,borderRadius:4}}><div style={{height:"100%",width:`${(count/(insightData.locations[0]?.[1]||1))*100}%`,background:GOLD,borderRadius:4}}/></div></div>):<div style={{color:MUTED,fontSize:14}}>Location insights will appear as viewers share a public location.</div>}</div>:<div style={{marginTop:18,padding:"20px 4px",color:MUTED,fontSize:14,lineHeight:1.5}}>Gender and age are not currently stored for RainX viewers. The analytics structure is ready for those fields without inventing demographic data.</div>}</Section>
        <Section title="Most active times"><div style={{fontSize:15,lineHeight:1.45,marginBottom:15}}>{tab==="Viewers"?"Your viewers were most active during the busiest tracked hour.":"Your followers were most active during the busiest tracked hour."}</div><div style={{display:"flex",alignItems:"flex-end",gap:3,height:160}}>{(insightData?.hours||[]).map(x=>{const max=Math.max(...(insightData?.hours||[]).map(a=>a.value),1);return <div key={x.hour} title={`${x.hour}:00 · ${fmt(x.value)}`} style={{flex:1,height:`${Math.max(3,x.value/max*140)}px`,background:"#CFE2F5",borderRadius:"3px 3px 0 0"}}/>})}</div><div style={{display:"flex",justifyContent:"space-between",color:MUTED,fontSize:11,marginTop:5}}><span>12a</span><span>4a</span><span>8a</span><span>12p</span><span>4p</span><span>8p</span></div></Section></>}
    </div>{loading&&<div style={{position:"absolute",top:68,right:14,fontSize:11,color:MUTED}}>Updating…</div>}
  </div></div>;
}
