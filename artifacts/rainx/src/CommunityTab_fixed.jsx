...
import {
  ...
  UserPlus, UserCheck, ArrowLeft, Bell, MoreHorizontal, Plus, Hash, AtSign, Flag, ChevronRight, ChevronDown, FileText, MessageSquare, Search, Bookmark, Share2,
} from "lucide-react";
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
          <div style={{ flex: 1 }}>
            <MentionTextarea
              ...
              style={{ width: "100%", minHeight: 38, boxSizing: "border-box", background: T.ink, border: `1px solid ${T.cardBorder}`, borderRadius: 18, color: T.paper, padding: "9px 13px", fontFamily: FONT_BODY, fontSize: 13, resize: "none", outline: "none", transition: "border-color .18s, box-shadow .18s" }}
            />
          </div>
          <button onClick={handleSubmit} disabled={!text.trim()} style={{ background: T.goldGradient, color: T.ink, border: "none", borderRadius: 16, minHeight: 38, padding: "0 15px", fontFamily: FONT_HEAD, fontWeight: 800, fontSize: 11.5, cursor: text.trim() ? "pointer" : "default", flexShrink: 0, opacity: text.trim() ? 1 : 0.5, boxShadow: text.trim() ? "0 4px 12px rgba(0,0,0,.14)" : "none", transition: "transform .16s ease, opacity .18s ease, box-shadow .18s ease" }} onMouseDown={(e) => { if (text.trim()) e.currentTarget.style.transform = "scale(.96)"; }} onMouseUp={(e) => { e.currentTarget.style.transform = "scale(1)"; }}>{replyTo ? "Reply" : "Post"}</button>
        </div>
      </div>
    </div>
  );
...
function PostCard({ post, profile, account, profilesMap, onProfilesNeeded, likeData, onToggleLike, repostData, onToggleRepost, onOpenProfile, onDelete, onHidePost, onEdit, onReport, onActivityOpen, onDmUser, forceOpen, onOpened }) {
  ...
  return (
    <div style={{ padding: "12px 16px", borderBottom: `1px solid ${T.cardBorder}`, animation: "fadeInUp 0.25s ease" }}>
      {isRepost && (
        <div style={{ display:"flex", alignItems:"center", gap:6, margin:"0 0 8px 2px", color:T.muted, fontSize:11.5, fontWeight:600 }}>
          <Repeat2 size={14} color={T.sage} strokeWidth={2.2} />
      ...
      <div style={isRepost ? { border:`1px solid ${T.cardBorder}`, borderRadius:16, padding:"12px 12px 11px", background:`${T.card}88`, boxShadow:"0 1px 0 rgba(255,255,255,.02)" } : undefined}>
      <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
      {/* Avatar column */}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:6, position:"relative" }}>
          {!isOwn && (
            <button onClick={toggleFollowUser} style={{ border:`1px solid ${isFollowing ? T.cardBorder : T.gold}`, background:isFollowing ? "transparent" : T.gold, color:isFollowing ? T.paper : T.ink, borderRadius:999, padding:"5px 10px", fontSize:10.5, fontWeight:800, cursor:"pointer" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 7, minWidth: 0 }}>
                  ...
                  <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:5, flexShrink:0 }}>
                    {!isOwn && (
                      <button onClick={toggleFollowUser} style={{ border:`1px solid ${isFollowing ? T.cardBorder : T.gold}`, background:isFollowing ? "transparent" : T.gold, color:isFollowing ? T.paper : T.ink, borderRadius:999, padding:"5px 10px", fontSize:10.5, fontWeight:800, cursor:"pointer", transition:"transform .15s ease, opacity .18s ease" }}>{isFollowing ? "Following" : "Follow"}</button>
                    )}
                    <button onClick={() => setMenuOpen(true)} style={{ background:"none", border:"none", color:T.muted, cursor:"pointer", display:"flex", padding:4 }}><MoreHorizontal size={18} /></button>
                  </div>
                </div>
                {menuOpen && (
                  <PostMenuSheet
                    ...
                    onDelete={() => { onDelete(post.id); setMenuOpen(false); }}
                    onReport={() => { onReport(post.id); setMenuOpen(false); }}
                    onNotInterested={() => { supabase.from("post_not_interested").insert({ user_id: account.id, post_id: post.id }).then(() => {}); onDelete(post.id); setMenuOpen(false); }}
                    isFollowing={isFollowing}
                    isMuted={isMuted}
                    isBlocked={isBlocked}
                    onFollow={() => { toggleFollowUser(); setMenuOpen(false); }}
      ...
      </div>{/* end repost wrapper / post content */}
      </div>{/* end post card */}
    </div>
  );
}
...
function ProfileView({ userId, account, onBack, onOpenProfile, onDmUser }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ data: postRows }, { data: repostRows }] = await Promise.all([
        ...
        supabase.from("post_reposts").select("post_id, created_at").eq("user_id", userId).order("created_at", { ascending: false }),
      ]);
      const profilePosts = postRows || [];
      let profileReposts = [];
      if ((repostRows || []).length) {
        ...
        profileReposts = (repostRows || []).map(r => {
  ...
  return (
    <div style={{ minHeight:"100%", background:T.ink, overflowY:"auto" }}>
      ...
      {/* ── Posts / Reposts ── */}
      <div style={{ borderTop:`1px solid ${T.cardBorder}`, display:"flex", alignItems:"stretch", marginTop:2 }}>
        {[
          { key:"posts", label:"Posts", Icon:FileText, count:posts.length },
          { key:"reposts", label:"Reposts", Icon:Repeat2, count:reposts.length },
        ].map(({ key, label, Icon }) => {
          const active = activeProfileTab === key;
          return (
            <button key={key} onClick={() => setActiveProfileTab(key)}
              style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", gap:7, padding:"13px 10px 11px", background:"none", border:"none", borderBottom:`2px solid ${active ? T.gold : "transparent"}`, color:active ? T.paper : T.muted, fontFamily:FONT_HEAD, fontWeight:active ? 800 : 600, fontSize:13, cursor:"pointer", transition:"color .18s, border-color .18s" }}>
      ...
      {(() => {
        const activePosts = activeProfileTab === "reposts" ? reposts : posts;
        const emptyLabel = activeProfileTab === "reposts" ? "No reposts yet." : "No posts yet.";
        return activePosts.length === 0 ? (
          <div style={{ fontSize:13, color:T.muted, padding:"38px 16px", textAlign:"center" }}>{emptyLabel}</div>
        ) : (
          <ProfileFeed posts={activePosts} account={account} profileEntry={profile} onOpenProfile={onOpenProfile} onDmUser={onDmUser} />
        );
      })()}
    </div>
  );
    ...
    <div style={{ position: "relative", minHeight: "100%", background: T.ink }}>
      ...
      {postsLoading && posts.length === 0 ? (
        ...
        <PostCard
          ...
          onOpenProfile={setViewingUserId} onDelete={deletePost} onHidePost={(id) => setPosts(prev => prev.filter(x => x.id !== id || x._isRepost))} onEdit={loadPosts} onReport={reportPost}
