      ...
      {suggestions.length > 0 && (
        <div style={{
          ...
          {suggestions.map((s) => (
            ...
            >
              <div style={{
                width: 28, height: 28, borderRadius: "50%", background: "#F4D35E",
              ...
              <span style={{ color: "#F4D35E", fontWeight: 700, fontSize: 13, fontFamily: "'Montserrat', sans-serif" }}>
                @{s.display_name}
              </span>
            </div>
          ))}
...
const GHANAIAN_SEED_NAMES = [
  "Kwame Mensah", "Kojo Asante", "Kofi Boateng", "Yaw Owusu",
  "Kwaku Addo", "Nana Osei", "Ama Serwaa", "Akua Mensah",
  "Abena Owusu", "Adwoa Asante", "Esi Boateng", "Afia Osei",
  "Kofi Mensah", "Kwesi Arthur", "Yaw Mensah", "Kojo Owusu"
];
function fallbackGhanaianName(id = "") {
  let hash = 0;
  for (let i = 0; i < id.length; i++) hash = ((hash << 5) - hash + id.charCodeAt(i)) | 0;
  return GHANAIAN_SEED_NAMES[Math.abs(hash) % GHANAIAN_SEED_NAMES.length];
}
...
async function fetchProfilesMap(ids) {
  ...
  uniqueIds.forEach((id) => {
    if (!map[id]) {
      const name = fallbackGhanaianName(id);
      map[id] = { id, display_name: name, full_name: name, username: name.toLowerCase().replace(/[^a-z0-9]+/g, "") };
    }
  });
  return map;
}
...
export function Badge({ isAdmin, badge, isPro, isOfficial }) {
  if (badge === "official" || isOfficial) return <OfficialBadge />;
...
async function togglePostLike(postId, authorId, accountId, likeData, setLikeData) {
  ...
  const next = { count: Math.max(0, cur.count + (cur.likedByMe ? -1 : 1)), likedByMe: !cur.likedByMe };
  ...
  try {
    ...
    // Read the committed counter once. The DB trigger is now atomic and is the
    // canonical source, so every surface converges to the same value.
    const { data: canonical } = await supabase.from("community_posts").select("likes_count").eq("id", postId).maybeSingle();
    if (canonical) setLikeData((d) => ({ ...d, [postId]: { count: Number(canonical.likes_count) || 0, likedByMe: next.likedByMe } }));
    if (!cur.likedByMe) notify(authorId, accountId, "like", postId);
  } finally {
    pendingPostLikeOps.delete(lockKey);
...
async function toggleCommentLike(commentId, authorId, postId, accountId, likeData, setLikeData) {
  ...
  const next = { count: Math.max(0, cur.count + (cur.likedByMe ? -1 : 1)), likedByMe: !cur.likedByMe };
  setLikeData((d) => ({ ...d, [commentId]: next }));
  const { error } = cur.likedByMe
  ...
  if (error && error.code !== "23505") {
    setLikeData((d) => ({ ...d, [commentId]: cur }));
    return;
  }
  if (!cur.likedByMe) notify(authorId, accountId, "comment_like", postId);
}
...
async function togglePostRepost(postId, authorId, accountId, repostData, setRepostData) {
  ...
  const next = { count: Math.max(0, cur.count + (cur.repostedByMe ? -1 : 1)), repostedByMe: !cur.repostedByMe };
  // Repost feedback is immediate; Supabase persistence happens underneath it.
  setRepostData((d) => ({ ...d, [postId]: next }));
  if (cur.repostedByMe) {
    const { data: wrapper } = await supabase.from("community_posts").select("id").eq("user_id", accountId).eq("repost_of_post_id", postId).maybeSingle();
    const { error: relationError } = await supabase.from("post_reposts").delete().eq("post_id", postId).eq("user_id", accountId);
    if (relationError) { setRepostData((d) => ({ ...d, [postId]: cur })); return; }
    if (wrapper?.id) await supabase.from("community_posts").delete().eq("id", wrapper.id).eq("user_id", accountId);
    return;
  }
  const { data: wrapper, error: wrapperError } = await supabase.from("community_posts").insert({ user_id:accountId, text:"", images:[], views:0, likes_count:0, comments_count:0, reposts_count:0, repost_of_post_id:postId }).select("*").single();
  if (wrapperError || !wrapper) { setRepostData((d) => ({ ...d, [postId]: cur })); return; }
  const { error: relationError } = await supabase.from("post_reposts").insert({ post_id:postId, user_id:accountId });
  if (relationError) { await supabase.from("community_posts").delete().eq("id", wrapper.id).eq("user_id", accountId); setRepostData((d) => ({ ...d, [postId]: cur })); return; }
...
function Composer({ account, onPosted, onClose, compact, themeTokens }) {
  ...
  const locationChip = location && (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "rgba(244,211,94,0.12)", border: `1px solid ${T.gold}44`, borderRadius: 20, padding: "4px 11px", marginTop: 8, cursor: "pointer" }} onClick={() => setLocation(null)}>
...
function CommentsSection({ postId, postAuthorId, account, profilesMap, onProfilesNeeded, onOpenProfile, onCommentsChange }) {
  ...
  useEffect(() => {
    if (!postId) return;
    const ch = supabase.channel("post-comments-live-" + postId)
      .on("postgres_changes", { event:"INSERT", schema:"public", table:"post_comments", filter:`post_id=eq.${postId}` }, ({ new: row }) => {
        if (!row?.id) return;
        setComments(prev => {
          const list = prev || [];
          if (list.some(c => c.id === row.id)) return list;
          const next = [...list, row];
          if (typeof onCommentsChange === "function") onCommentsChange(next.length);
          return next;
        });
        onProfilesNeeded([row.user_id]);
      })
      .on("postgres_changes", { event:"DELETE", schema:"public", table:"post_comments", filter:`post_id=eq.${postId}` }, ({ old: row }) => {
        if (!row?.id) return;
        setComments(prev => {
  ...
  }, [postId, onProfilesNeeded]);

  const submitComment = async () => {
    if (!text.trim()) return;
    const trimmed = text.trim();
    const tempId = "temp_comment_" + Date.now();
    setComments(prev => [...(prev || []), { id:tempId, post_id:postId, user_id:account.id, text:trimmed, created_at:new Date().toISOString(), _pending:true }]);
    setLikeData(prev => ({ ...prev, [tempId]: { count:0, likedByMe:false } }));
    onProfilesNeeded([account.id]);
    setText("");
    const { data: c, error: commentError } = await supabase.from("post_comments").insert({ post_id: postId, user_id: account.id, text: trimmed }).select("*").single();
    if (commentError || !c) {
...
function PostMenuSheet({ isOwn, username, isFollowing = false, isMuted = false, isBlocked = false, onClose, onEdit, onDelete, onReport, onNotInterested, onUnfollow, onFollow, onMute, onUnmute, onBlock, onUnblock }) {
  const ownItems = [
  ...
  const otherItems = [
    ...
    { icon: isFollowing ? <UserCheck size={18} /> : <UserPlus size={18} />, label: isFollowing ? `Unfollow @${username}` : `Follow @${username}`, action: isFollowing ? onUnfollow : onFollow, danger: false },
    { icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 5h2M12 5v14m-7-7h2a5 5 0 0 1 5-5"/><path d="M5 19l14-14"/></svg>, label: isMuted ? `Unmute @${username}` : `Mute @${username}`, action: isMuted ? onUnmute : onMute, danger: false },
    { icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M4.93 4.93l14.14 14.14"/></svg>, label: isBlocked ? `Unblock @${username}` : `Block @${username}`, action: isBlocked ? onUnblock : onBlock, danger: false },
      ...
      {options.map((opt, i) => {
        ...
        return (
          <button key={opt.id} onClick={() => castVote(opt.id)} disabled={!!myVote} style={{ position: "relative", width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "11px 14px", background: "none", border: "none", borderBottom: i < options.length - 1 ? `1px solid ${T.cardBorder}` : "none", cursor: myVote ? "default" : "pointer", overflow: "hidden", textAlign: "left" }}>
            {myVote && <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: `${pct}%`, background: isChosen ? "rgba(244,211,94,0.22)" : "rgba(242,237,224,0.07)", transition: "width 0.5s ease", borderRadius: i === 0 ? "13px 0 0 0" : i === options.length - 1 ? "0 0 0 13px" : 0 }} />}
...
function PostCard({ post, profile, account, profilesMap, onProfilesNeeded, likeData, onToggleLike, repostData, onToggleRepost, onOpenProfile, onOpenPost, onDelete, onHidePost, onEdit, onReport, onActivityOpen, onDmUser, forceOpen, onOpened }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await supabase.from("community_posts").select("*").eq("id", post.repost_of_post_id).maybeSingle();
      if (cancelled || !data) return;
      setOriginalPost(data);
      if (profilesMap?.[data.user_id]) {
        setOriginalProfile(profilesMap[data.user_id]);
      } else {
        const { data: p } = await supabase.from("public_profiles")
          .select("id,display_name,full_name,username,avatar_url,is_admin,badge")
    ...
    return () => { cancelled = true; };
  }, [post.repost_of_post_id, originalPost?.id, profilesMap, account.id]);
  const isOwn = post.user_id === account.id;
  useEffect(() => {
    if (!account?.id || isOwn) return;
    (async () => {
      const [{ data: f }, { data: m }, { data: b }] = await Promise.all([
        supabase.from("follows").select("followed_id").eq("follower_id", account.id).eq("followed_id", post.user_id).maybeSingle(),
        supabase.from("user_mutes").select("muted_id").eq("muter_id", account.id).eq("muted_id", post.user_id).maybeSingle(),
        supabase.from("user_blocks").select("blocked_id").eq("blocker_id", account.id).eq("blocked_id", post.user_id).maybeSingle(),
      ]);
  ...
  const setMuteState = async () => {
    if (isMuted) { await supabase.from("user_mutes").delete().eq("muter_id", account.id).eq("muted_id", post.user_id); setIsMuted(false); }
    else { const { error } = await supabase.from("user_mutes").insert({ muter_id: account.id, muted_id: post.user_id }); if (!error) { setIsMuted(true); onHidePost?.(post.id); setShowComments(false); } }
  };
  ...
  // One live counter for the card and the open comments panel.
  const feedCommentCount = commentCount;
        ...
        <div style={{ display:"flex", alignItems:"center", gap:6, position:"relative" }}>
          ...
          {menuOpen && (
            <PostMenuSheet
              ...
              isMuted={isMuted}
              isBlocked={isBlocked}
              onFollow={() => { toggleFollowUser(); setMenuOpen(false); }}
              onUnfollow={() => { toggleFollowUser(); setMenuOpen(false); }}
              onMute={() => { setMuteState(); setMenuOpen(false); }}
              onUnmute={() => { setMuteState(); setMenuOpen(false); }}
              onBlock={() => { setBlockState(); setMenuOpen(false); }}
              onUnblock={() => { setBlockState(); setMenuOpen(false); }}
            />
          )}
        </div>
      </div>
            ...
            {originalPost ? (
              <>
                <div style={{ display:"flex", alignItems:"center", gap:9, marginBottom:10 }}>
                  <Avatar name={originalProfile?.display_name || originalProfile?.full_name || originalProfile?.username} avatarUrl={originalProfile?.avatar_url} size={34} />
                  <div style={{ minWidth:0, flex:1 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:4, minWidth:0 }}>
                      <span style={{ fontSize:13.5, fontWeight:800, color:T.paper, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{originalProfile?.full_name || originalProfile?.display_name || originalProfile?.username || "User"}</span>
                      <Badge isAdmin={originalProfile?.is_admin} badge={originalProfile?.badge} />
                      <span style={{ fontSize:10.5, color:T.muted }}>· {timeAgo(originalPost.created_at)}</span>
                    </div>
                    <div style={{ fontSize:10.5, color:T.muted }}>@{originalProfile?.username || originalProfile?.display_name || "user"}</div>
                  </div>
                </div>
                <div style={{
                  fontSize:14.5, fontWeight:500, color:T.paper, lineHeight:1.5, whiteSpace:"pre-wrap",
                  fontFamily:"'Montserrat', sans-serif", letterSpacing:0.1,
                ...
                {((originalPost.text || "").length > 260) && (
                  <span style={{ display:"inline-block", marginTop:7, color:T.gold, fontSize:12.5, fontWeight:800 }}>See more</span>
                )}
                {originalPost.images?.length > 0 && (
                  <div style={{ marginTop:10, borderRadius:12, overflow:"hidden", maxHeight:210 }}>
                    <img src={originalPost.images[0]} alt="Original post attachment" style={{ width:"100%", maxHeight:210, objectFit:"cover", display:"block" }} />
                  </div>
                )}
              </>
            ) : (
                ...
                <div style={{ display:"flex", gap:8, marginBottom:10 }}>
                  {["1","5","10","20","50"].map(v => (
                    <button key={v} onClick={() => setAmount(v)}
                      style={{ flex:1, padding:"8px 0", borderRadius:10, border:`1px solid ${amount===v?T.gold:T.cardBorder}`, background:amount===v?"rgba(244,211,94,0.15)":"none", color:amount===v?T.gold:T.paper, fontFamily:FONT_HEAD, fontWeight:700, fontSize:12, cursor:"pointer" }}>
                      ${v}
                    </button>
                  ))}
                </div>
...
function FollowListModal({ userId, type, viewerId, onClose, onOpenProfile }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (type === "followers") {
          let query = supabase.from("follows").select("follower_id").eq("followed_id", userId).order("created_at", { ascending: false });
          if (!isOwner) query = query.limit(50);
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:100, display:"flex", alignItems:"flex-end" }} onClick={onClose}>
      <div style={{ background:T.card, width:"100%", maxWidth:480, margin:"0 auto", borderRadius:"20px 20px 0 0", padding:"18px 16px 28px", maxHeight:"70vh", display:"flex", flexDirection:"column" }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ overflowY:"auto", flex:1 }}>
          {type === "followers" && viewerId !== userId && rows && rows.length >= 50 && (
            <div style={{ margin:"0 0 10px", padding:"10px 12px", borderRadius:12, background:`${T.gold}14`, border:`1px solid ${T.gold}40`, color:T.muted, fontSize:11.5, lineHeight:1.45 }}>
              Only the first 50 followers are visible. Only the account owner can see the remaining followers.
            </div>
          )}
          {rows === null && !loadError && (
          ...
          {rows !== null && !loadError && rows.map(({ uid, profile: p }) => {
            const displayName = p?.display_name || p?.full_name || p?.username || fallbackGhanaianName(uid);
            return (
...
function ProfileView({ userId, account, onBack, onOpenProfile, onDmUser }) {
  const [profile, setProfile] = useState(() => userId === account?.id ? {
    id: account?.id, display_name: account?.display_name || account?.full_name || account?.username || "RainX User",
    full_name: account?.full_name || account?.display_name || account?.username || "RainX User", username: account?.username || "",
    avatar_url: account?.avatar_url || null, badge: account?.badge || (account?.is_official ? "official" : null), is_official: !!account?.is_official,
  } : null);
  ...
  const [profileOpenPostId, setProfileOpenPostId] = useState(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const canonical = canonicalResult?.data || null;
        setProfile({ ...(canonical || {}), ...data, badge: data?.badge || canonical?.badge || (canonical?.is_official ? "official" : null), is_official: !!(data?.is_official || canonical?.is_official) });
      } catch (apiErr) {
        ...
        const { data: directP } = await supabase.from("profiles")
          .select("id,cover_url,location,full_name,username,display_name,date_of_birth,dob_privacy,avatar_url,bio,is_admin,badge,is_official")
          .eq("id", userId).single();
      ...
      const ownReposts = (postRows || []).filter(p => p.repost_of_post_id);
      const originalIds = [...new Set(ownReposts.map(p => p.repost_of_post_id).filter(Boolean))];
      let originalById = {};
      let originalProfiles = {};
      if (originalIds.length) {
        const { data: originals } = await supabase.from("community_posts").select("*").in("id", originalIds);
        originalById = Object.fromEntries((originals || []).map(p => [p.id, p]));
        const originalUserIds = [...new Set((originals || []).map(p => p.user_id).filter(Boolean))];
        originalProfiles = originalUserIds.length ? await fetchProfilesMap(originalUserIds) : {};
      }

      const decoratedPosts = (postRows || []).map(p => p.repost_of_post_id ? {
        ...p,
      ...
      const { data: blockRow } = await supabase.from("user_blocks").select("blocked_id").eq("blocker_id", account.id).eq("blocked_id", userId).maybeSingle();
      const { data: muteRow } = await supabase.from("user_mutes").select("muted_id").eq("muter_id", account.id).eq("muted_id", userId).maybeSingle();
      setIsFollowing(!!mine);
  ...
  useEffect(() => {
    if (!userId) return;
    const channel = supabase.channel("profile-live-" + userId)
      .on("postgres_changes", { event:"INSERT", schema:"public", table:"community_posts", filter:`user_id=eq.${userId}` }, ({ new: row }) => {
        if (!row?.id) return;
        setPosts(prev => { const list = prev || []; return list.some(p => p.id === row.id) ? list : [row, ...list]; });
      })
      .on("postgres_changes", { event:"UPDATE", schema:"public", table:"community_posts", filter:`user_id=eq.${userId}` }, ({ new: row }) => {
        if (!row?.id) return;
        setPosts(prev => (prev || []).map(p => p.id === row.id ? { ...p, ...row } : p));
      })
      .on("postgres_changes", { event:"DELETE", schema:"public", table:"community_posts", filter:`user_id=eq.${userId}` }, ({ old: row }) => {
        if (!row?.id) return;
  ...
  const toggleProfileBlock = async () => {
    if (isBlocked) { const { error } = await supabase.from("user_blocks").delete().eq("blocker_id", account.id).eq("blocked_id", userId); if (!error) setIsBlocked(false); }
    else { const { error } = await supabase.from("user_blocks").insert({ blocker_id: account.id, blocked_id: userId }); if (!error) { await supabase.from("follows").delete().eq("follower_id", account.id).eq("followed_id", userId); setIsBlocked(true); setIsFollowing(false); } }
    setShowSafetyMenu(false);
  };
  const toggleProfileMute = async () => {
    if (isMuted) { const { error } = await supabase.from("user_mutes").delete().eq("muter_id", account.id).eq("muted_id", userId); if (!error) setIsMuted(false); }
    else { const { error } = await supabase.from("user_mutes").insert({ muter_id: account.id, muted_id: userId }); if (!error) setIsMuted(true); }
    setShowSafetyMenu(false);
  };
  ...
  return (
    <div style={{ minHeight:"100%", background:T.ink, overflowY:"auto" }}>
      ...
      {/* ── Modals ── */}
      {showFollowList && (
        <FollowListModal userId={userId} type={showFollowList} viewerId={account.id} onClose={() => setShowFollowList(null)} onOpenProfile={onOpenProfile} />
      )}
      {showGift && (
        <GiftModal profile={profile} senderAccount={account} onClose={() => setShowGift(false)} />
      )}
      {showSafetyMenu && !isOwnProfile && (
        <div onClick={() => setShowSafetyMenu(false)} style={{ position:"fixed", inset:0, zIndex:400, background:"rgba(0,0,0,.5)" }}>
          <div onClick={e => e.stopPropagation()} style={{ position:"absolute", bottom:0, left:0, right:0, maxWidth:480, margin:"0 auto", background:T.card, borderRadius:"22px 22px 0 0", padding:"10px 16px 30px", boxShadow:"0 -12px 35px rgba(0,0,0,.25)" }}>
            <div style={{ width:38, height:4, borderRadius:4, background:T.cardBorder, margin:"2px auto 14px" }} />
            <button onClick={toggleProfileMute} style={{ width:"100%", padding:"14px 8px", border:0, background:"none", color:T.paper, textAlign:"left", fontSize:14, fontWeight:700, cursor:"pointer" }}>{isMuted ? `Unmute @${handle}` : `Mute @${handle}`}</button>
            <div style={{ height:1, background:T.cardBorder }} />
            <button onClick={toggleProfileBlock} style={{ width:"100%", padding:"14px 8px", border:0, background:"none", color:isBlocked ? T.paper : T.rust, textAlign:"left", fontSize:14, fontWeight:700, cursor:"pointer" }}>{isBlocked ? `Unblock @${handle}` : `Block @${handle}`}</button>
          </div>
        </div>
      )}
      ...
      <div style={{ position:"relative", animation:"pvSlideIn 0.22s ease" }}>
        ...
        {!isOwnProfile && (
          <div style={{ position:"absolute", bottom:-44, right:14, display:"flex", alignItems:"center", gap:8 }}>
            ...
            <button onClick={() => setShowSafetyMenu(true)} title="More" style={{ width:38, height:38, borderRadius:"50%", background:"none", border:`1.5px solid ${T.cardBorder}`, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", flexShrink:0 }}>
              <MoreHorizontal size={17} color={T.paper} />
            </button>
            {/* Follow */}
      ...
      {(() => {
        const tabPosts = profileTab === "reposts" ? posts.filter(p => !!p.repost_of_post_id) : posts.filter(p => !p.repost_of_post_id);
        if (!tabPosts.length) {
          return <div style={{ fontSize:13, color:T.muted, padding:"32px 0", textAlign:"center" }}>{profileTab === "reposts" ? "No reposts yet." : "No posts yet."}</div>;
        }
        return <ProfileFeed posts={tabPosts} account={account} profileEntry={profile} onOpenProfile={onOpenProfile} onOpenPost={setProfileOpenPostId} forceOpenPostId={profileOpenPostId} onDmUser={onDmUser} />;
      })()}
    </div>
  );
}
...
function ProfileFeed({ posts, account, profileEntry, onOpenProfile, onOpenPost, forceOpenPostId, onDmUser, onDelete, onRefresh, themeTokens }) {
  if (themeTokens) Object.assign(T, themeTokens);
  const [likeData, setLikeData] = useState({});
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      ids.forEach((id) => {
        ...
        const pending = pendingPostLikeOps.has(`${account.id}:${id}`);
        const existing = likeData[id];
        ld[id] = {
          count: pending && existing ? existing.count : (Number(post?.likes_count) || 0),
          likedByMe: pending && existing ? existing.likedByMe : myLikedSet.has(id),
        };
      });
      setLikeData(ld);
    ...
    <div>
      {posts.map(p => (
        <PostCard
          ...
          forceOpen={forceOpenPostId === p.id}
          onOpened={() => onOpenPost?.(null)}
          onDelete={onDelete || (() => {})}
          onEdit={onRefresh || (() => {})}
          onReport={() => {}}
          onDmUser={onDmUser}
...
export default function CommunityTab({ account, entitlement, themeTokens, onViewingProfileChange }) {
  ...
  const loadPosts = useCallback(async () => {
    ...
    if (repostWrappers.length) {
      const originalIds = [...new Set(repostWrappers.map(p => p.repost_of_post_id).filter(Boolean))];
      const { data: originals } = await supabase.from("community_posts").select("*").in("id", originalIds);
      const originalById = Object.fromEntries((originals || []).map(p => [p.id, p]));
      const originalUserIds = [...new Set((originals || []).map(p => p.user_id).filter(Boolean))];
      const originalProfiles = originalUserIds.length ? await fetchProfilesMap(originalUserIds) : {};
  ...
  useEffect(() => {
    const channel = supabase.channel("community-live-feed")
      .on("postgres_changes", { event:"INSERT", schema:"public", table:"community_posts" }, async ({ new: row }) => {
        if (!row?.id || row.user_id === account.id) return;
        const map = await fetchProfilesMap([row.user_id]);
        setPosts(prev => prev.some(p => p.id === row.id) ? prev : [{ ...row, _originalPost:null, _originalProfile:null }, ...prev]);
        if (map?.[row.user_id]) setProfilesMap(prev => ({ ...prev, [row.user_id]: map[row.user_id] }));
        setLikeData(prev => prev[row.id] ? prev : { ...prev, [row.id]: { count:Number(row.likes_count)||0, likedByMe:false } });
        setRepostData(prev => prev[row.id] ? prev : { ...prev, [row.id]: { count:Number(row.reposts_count)||0, repostedByMe:false } });
      })
      .on("postgres_changes", { event:"UPDATE", schema:"public", table:"community_posts" }, ({ new: row }) => {
        ...
        setLikeData(prev => {
          ...
          const pending = pendingPostLikeOps.has(`${account.id}:${row.id}`);
          return pending ? prev : { ...prev, [row.id]: { ...prev[row.id], count:Number(row.likes_count)||0 } };
        });
        setRepostData(prev => prev[row.id] ? { ...prev, [row.id]: { ...prev[row.id], count:Number(row.reposts_count)||0 } } : prev);
      })
      .on("postgres_changes", { event:"DELETE", schema:"public", table:"community_posts" }, ({ old: row }) => {
        if (!row?.id) return;
        setPosts(prev => prev.filter(p => p.id !== row.id));
        setLikeData(prev => { const n={...prev}; delete n[row.id]; return n; });
        setRepostData(prev => { const n={...prev}; delete n[row.id]; return n; });
      })
      .subscribe();
    return () => { try { supabase.removeChannel(channel); } catch(_){} };
  }, [account?.id]);
  ...
  return (
    <div style={{ position: "relative", minHeight: "100%", background: T.ink }}>
      ...
      {postsLoading && posts.length === 0 ? (
        ...
        <PostCard
          key={p._feedKey || p.id} post={p} profile={profilesMap[p.user_id]} account={account}
