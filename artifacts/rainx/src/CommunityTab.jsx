...
async function togglePostRepost(postId, authorId, accountId, repostData, setRepostData) {
  ...
  if (cur.repostedByMe) {
    // A repost is its own community post. Remove both the relationship and
    // the user's repost-post wrapper.
    ...
    if (relationError) {
      if (process.env.NODE_ENV !== "production") console.error("Unrepost failed:", relationError.message);
      return;
    }
    ...
    if (wrapper?.id) {
      const { error: wrapperError } = await supabase
        ...
        .eq("id", wrapper.id)
        .eq("user_id", accountId);
      if (wrapperError && process.env.NODE_ENV !== "production") {
        console.error("Repost post cleanup failed:", wrapperError.message);
      }
    }
  ...
  // Create the repost as a real post owned by the person reposting.
  ...
  if (wrapperError || !wrapper) {
    if (process.env.NODE_ENV !== "production") console.error("Repost post creation failed:", wrapperError?.message);
    return;
  }

  const { error: relationError } = await supabase
    .from("post_reposts")
    .insert({ post_id: postId, user_id: accountId });

  if (relationError) {
    await supabase.from("community_posts").delete().eq("id", wrapper.id).eq("user_id", accountId);
    if (process.env.NODE_ENV !== "production") console.error("Repost relation failed:", relationError.message);
    return;
  }
