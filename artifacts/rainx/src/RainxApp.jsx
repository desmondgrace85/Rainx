...
function ReferralRewardsScreen({ count, earnings, referralCode, account, onBack, onActivity }) {
  ...
  const startSwipe=e=>{if(e.touches.length===1)touch.current={x:e.touches[0].clientX,y:e.touches[0].clientY};}; const endSwipe=e=>{if(!touch.current)return; const dx=e.changedTouches[0].clientX-touch.current.x,dy=e.changedTouches[0].clientY-touch.current.y; touch.current=null; if(Math.abs(dx)>45&&Math.abs(dx)>Math.abs(dy))setPage(p=>Math.max(0,Math.min(2,p+(dx<0?1:-1))));};
  ...
  return createPortal(<div style={{position:"fixed",inset:0,zIndex:1000,background:"#EEF0F1",overflow:"hidden",color:"#17191B",fontFamily:"-apple-system,BlinkMacSystemFont,\"SF Pro Display\",\"SF Pro Text\",Arial,sans-serif"}}><div style={{width:"100%",height:"100%",maxWidth:760,margin:"auto",background:"#FCFDFD",display:"flex",flexDirection:"column",overflow:"hidden"}}><header style={{height:58,minHeight:58,background:"#fff",display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}><button onClick={onBack} aria-label="Back" style={{position:"absolute",left:14,width:36,height:36,border:0,background:"none"}}><ChevronLeft size={22}/></button><div style={{fontSize:23,fontWeight:750}}>My Referrals</div></header><main onTouchStart={startSwipe} onTouchEnd={endSwipe} style={{flex:1,minHeight:0,overflowY:"auto",padding:"28px 28px 24px"}}><div style={{width:"100%",height:190,borderRadius:24,overflow:"hidden",boxShadow:"0 6px 14px rgba(30,35,38,.07)"}}><img src={referralActivityBanner} alt="Earn from referrals" style={{width:"100%",height:"100%",display:"block",objectFit:"cover"}}/></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,marginTop:20}}><button style={{height:40,border:0,borderRadius:21,color:"#fff",background:"#3159E9",fontSize:16,fontWeight:650}}>Request</button><button style={{height:40,border:0,borderRadius:21,color:"#fff",background:"#3B3B3C",fontSize:16,fontWeight:650}}>Pay</button></div><div style={{marginTop:24,overflow:"hidden"}}><div style={{height:42,display:"grid",gridTemplateColumns:"1fr 1.55fr 1.35fr 36px",alignItems:"center",borderBottom:"1px solid #E7E8E9"}}>{tabs.map((tab,index)=><button key={tab} onClick={()=>setPage(index)} style={{height:42,border:0,background:page===index?"#EEF0F2":"transparent",borderRadius:20,color:page===index?"#17191B":"#A7A9AC",fontSize:14.5,fontWeight:page===index?700:500,whiteSpace:"nowrap"}}>{tab}</button>)}<button aria-label="Refresh" onClick={()=>window.location.reload()} style={{height:42,border:0,background:"none"}}><RefreshCw size={20}/></button></div><div style={{display:"flex",width:"300%",transform:"translate3d("+(-page*33.3333)+"%,0,0)",transition:"transform 340ms cubic-bezier(.22,.8,.25,1)"}}><section style={{flex:"0 0 33.3333%",paddingTop:4}}>{shown.map(rowView)}</section><section style={{flex:"0 0 33.3333%",paddingTop:4}}>{active.map(rowView)}</section><section style={{flex:"0 0 33.3333%"}}><div style={{marginTop:18,padding:18,borderRadius:18,background:"#F3F4F4",display:"flex",flexDirection:"column",gap:5}}><span style={{fontSize:13,color:"#8D9092"}}>Total earned</span><strong style={{fontSize:22}}>GHS {total.toFixed(2)}</strong></div></section></div></div></main></div></div>,document.body);
}
