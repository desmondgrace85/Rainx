...
function MyReferralsScreen({ count, earnings, referralCode, account, onBack, onActivity }) {
  const [rows,setRows]=useState([]); const [page,setPage]=useState(0); const [claimed,setClaimed]=useState(() => new Set()); const [closing,setClosing]=useState(false); const [swipePage,setSwipePage]=useState(null); const [tabMetrics,setTabMetrics]=useState([]); const touch=useRef(null); const tabStrip=useRef(null); const tabRefs=useRef([]);
