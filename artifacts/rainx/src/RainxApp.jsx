...
export function ReferralRewardsScreen({ count, earnings, referralCode, onBack, onActivity }) {
  ...
  >
    ...
    <div style={{maxWidth:480,minHeight:"100dvh",margin:"0 auto",padding:"10px 22px 34px",boxSizing:"border-box",background:"#FFFFFF",willChange:"transform",animation:closing ? "referralSheetOut .52s cubic-bezier(.22,1,.36,1) both" : "referralSheetIn .52s cubic-bezier(.22,1,.36,1) both"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:48,position:"relative",marginBottom:2}}>
        ...
        <button type="button" onClick={() => { if (closing) return; setClosing(true); window.setTimeout(onActivity, 520); }} aria-label="View referrals" style={{position:"absolute",right:-2,width:34,height:34,borderRadius:"50%",border:0,background:"#17191B",display:"grid",placeItems:"center",cursor:"pointer"}}><Coins size={17} color="#FFFFFF" strokeWidth={2}/></button>
      </div>
...
export function MyReferralsScreen({ count, earnings, referralCode, account, onBack, onActivity }) {
  const [rows,setRows]=useState([]); const [refreshKey,setRefreshKey]=useState(0); const [page,setPage]=useState(0); const [claimed,setClaimed]=useState(() => new Set()); const [closing,setClosing]=useState(false); const [swipePage,setSwipePage]=useState(null); const [tabMetrics,setTabMetrics]=useState([]); const touch=useRef(null); const tabStrip=useRef(null); const tabRefs=useRef([]);
