...
function GiftsRewardsScreen({ onBack, account, settingsOpen = false, onOpenSettings, onBackFromSettings, underlying = false }) {
  ...
  const closeSettings = () => { if (settingsClosing) return; setSettingsClosing(true); window.setTimeout(() => { setSettingsClosing(false); onBackFromSettings?.(); }, 360); };
...
export default function MoreLandingOverride({ account }) {
  ...
  const giftScreen = moreOverlay === "gifts" || moreOverlay === "gifts-settings";
  const referralPage = moreOverlay === "referrals" ? "referrals" : moreOverlay === "referral-activity" ? "activity" : moreOverlay === "referral-performance" ? "performance" : null;
  ...
  return (
    ...
    >
      ...
      {giftScreen && moreOverlay === "gifts-settings" && <GiftsRewardsScreen underlying settingsOpen={false} onOpenSettings={() => openOverlay("gifts-settings")} onBackFromSettings={backOverlay} onBack={backOverlay} account={account} />}
