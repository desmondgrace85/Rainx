...
function GiftsRewardsScreen({ onBack, account }) {
  ...
  useEffect(() => { if (!account?.id) return; supabase.from("follows").select("id", { count: "exact", head: true }).eq("followed_id", account.id).then(({ count }) => setFollowers(count || 0)).catch(() => {}); }, [account?.id]);
  const requirements = [
    [followers >= 1000, "Have at least 1,000 followers"],
    [false, "Videos must pass safety moderation to be eligible for Gifts"],
    [false, "Have an active subscription for at least 30 days"],
    [false, "Meet the age requirement"],
    [false, "Accounts not used by government, politicians and political parties"],
  ];
